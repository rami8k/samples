resource "aws_lambda_function" "authorizer" {
  function_name    = "BU-edv-jwt-authorizer"
  handler          = "index.handler"
  runtime          = "nodejs14.x"
  role             = aws_iam_role.lambda_execution_role.arn
  filename         = "../jwt-authorizer-lambda.zip"
  source_code_hash = filebase64sha256("../jwt-authorizer-lambda.zip")
  tracing_config {
    mode = "Active"
  }
  tags = var.tags
...
resource "aws_cloudwatch_log_group" "authorizer_lambda_log_group" {
  name              = "/aws/lambda/BU-edv-jwt-authorizer"
